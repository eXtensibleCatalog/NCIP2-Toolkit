...
public class TestMonetaryValue {
    ...
    public void testFormatting() {
        ...
        String formattedJPY = NCIPHelper.formatMonetaryAmount(moneyMoneyMoney, jpyCC);
        Assert.assertEquals("12345", formattedJPY);

        NumberFormat jpyCurrencyNumberFormat = NumberFormat.getCurrencyInstance(Locale.JAPAN);
        String jpyFormattedWithSymbol = jpyCurrencyNumberFormat.format(NCIPHelper.decimalize(moneyMoneyMoney, jpyCC));
        ...
        String formattedUSD = NCIPHelper.formatMonetaryAmount(moneyMoneyMoney, usdCC);
        Assert.assertEquals("123.45", formattedUSD);
        NumberFormat usdCurrencyNumberFormat = NumberFormat.getCurrencyInstance(Locale.US);
        String usdFormattedWithSymbol = usdCurrencyNumberFormat.format(NCIPHelper.decimalize(moneyMoneyMoney, usdCC));
        Assert.assertEquals("$123.45", usdFormattedWithSymbol);

        CurrencyCode bhdCC = Version1CurrencyCode.BHD;
        String formattedBHD = NCIPHelper.formatMonetaryAmount(moneyMoneyMoney, bhdCC);
        ...
        String bhdFormattedWithSymbol = bhdCurrencyNumberFormat.format(NCIPHelper.decimalize(moneyMoneyMoney, bhdCC));
