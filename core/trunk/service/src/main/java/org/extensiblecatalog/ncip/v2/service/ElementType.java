/**
 * Copyright (c) 2010 eXtensible Catalog Organization
 *
 * This program is free software; you can redistribute it and/or modify it
 * under the terms of the MIT/X11 license. The text of the license can be
 * found at http://www.opensource.org/licenses/mit-license.php. 
 */

package org.extensiblecatalog.ncip.v2.service;

import org.apache.log4j.Logger;

import java.util.List;
import java.util.concurrent.CopyOnWriteArrayList;

/**
 * Identifies an Element, such as Account Details.
 */
public class ElementType extends SchemeValuePair {

    private static final Logger LOG = Logger.getLogger(ElementType.class);

    private static final List<ElementType> VALUES_LIST = new CopyOnWriteArrayList<ElementType>();

    public ElementType(String scheme, String value) {
        super(scheme, value);
        VALUES_LIST.add(this);
    }

    /**
     * Find the ElementType that matches the scheme & value strings supplied.
     *
     * @param scheme a String representing the Scheme URI.
     * @param value  a String representing the Value in the Scheme.
     * @return a ElementType that matches, or null if none is found to match.
     */
    public static ElementType find(String scheme, String value) throws ServiceException {
        return (ElementType) find(scheme, value, VALUES_LIST, ElementType.class);
    }

}
