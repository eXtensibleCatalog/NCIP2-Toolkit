...
public class AlephItemFactory implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2944867720594724165L;
	/*public static AlephItem getAlephItemByAdmId(String adm_base, String bib_base, String admId, 
			String xServerName, String xServerPort, boolean sslEnabled) 
	throws IOException,ParserConfigurationException,SAXException,AlephException{
		AlephItem item = new AlephItem();
		//first need to get bib id from adm id
		XService xService = XServiceFactory.createFindDocXService(adm_base, admId);
		Document doc = xService.execute(xServerName, xServerPort, sslEnabled);
		
		item = AlephItemFactory.updateAlephItemParseFindDocResponse(item, doc);
		
		//call again with bib id if found to get everything else
		if (item.getBibId()!=null){
			item = getAlephItemByBibId(bib_base, item.getBibId(), xServerName, xServerPort, sslEnabled);
		}
	 ...
	 */
	/*public static AlephItem getAlephItemByBibId(String library_base, String bibId, 
			String xServerName, String xServerPort, boolean sslEnabled) 
	throws IOException,ParserConfigurationException,SAXException,AlephException{
		AlephItem item = new AlephItem();
		item.setBibId(bibId);
		...
		//get bib info
		XService xService = XServiceFactory.createFindDocXService(library_base, bibId);
		Document doc = xService.execute(xServerName, xServerPort, sslEnabled);
		
		item = AlephItemFactory.updateAlephItemParseFindDocResponse(item, doc);
		
		//get circ status from item data call
		xService = XServiceFactory.createItemDataXService(library_base, bibId);
		doc = xService.execute(xServerName, xServerPort, sslEnabled);
