...
public class AlephMediator implements Serializable {
	...
	/**
	 * 
	 * Call the Aleph authentication process in order to authenticate a patron against the default agency,
	...
	/**
	 * 
	 * Call the Aleph authentication process in order to authenticate a patron against the agency
	...
	public AlephItem lookupItemByBibId(String bibId, String agencyId, boolean getBibInformation, 
		...
		AlephAPI AlephAPI = AlephAPIFactory.createItemDataAlephAPI(agency.getBibLibrary(), bibId);
		Document doc = AlephAPI.execute(getAlephName(), getAlephPort(), false);
		item = AlephItemFactory.updateAlephItemParseItemDataResponse(item, doc);
		
		if (getBibInformation){
			AlephAPI = AlephAPIFactory.createFindDocAlephAPI(agency.getBibLibrary(), bibId);
	...
	public List<AlephItem> lookupItemsByHoldingsId(String holdingsId, String agencyId, boolean getBibInformation, 
		...
		AlephAPI AlephAPI = AlephAPIFactory.createFindDocAlephAPI(agency
				.getHoldingsLibrary(), holdingsId);
		Document doc = AlephAPI.execute(getAlephName(), getAlephPort(),
				false);
		item = AlephItemFactory.updateAlephItemParseFindDocResponse(item, doc);
	...
	private Date renewItem(String request_user_bor_id, String adm_library, AlephItem item) 
		...
		if (item!=null&&item.getBarcode()!=null){
			AlephAPI AlephAPI = AlephAPIFactory.createRenewAlephAPI(adm_library, item.getBarcode(), request_user_bor_id);
			Document doc = AlephAPI.execute(getAlephName(), getAlephPort(), false);
			String error = XMLParserUtil.getError(doc);
			if (error!=null){
				if (item.getItemId()!=null){
	...
	private Map<String,AlephItem> getCircStatusByBibId(AlephAgency agency, String bibId) 
        ...
        AlephAPI AlephAPI = AlephAPIFactory.createCircStatusAlephAPI(agency.getBibLibrary(), bibId);
