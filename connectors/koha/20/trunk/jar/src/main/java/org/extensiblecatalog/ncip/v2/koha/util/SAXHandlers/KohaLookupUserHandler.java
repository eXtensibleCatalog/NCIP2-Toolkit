...
public class KohaLookupUserHandler extends DefaultHandler {
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		...
		if (parsingAddress) {
			if (qName.equalsIgnoreCase(KohaConstants.Z304_ADDRESS_1_NODE) && userIdDesired) {
				z304address1Reached = true;
			} else if (qName.equalsIgnoreCase(KohaConstants.Z304_ADDRESS_2_NODE) && nameInformationDesired) {
				z304address2Reached = true;
			} else if (userAddressInformationDesired) {
				if (qName.equalsIgnoreCase(KohaConstants.Z304_ADDRESS_3_NODE)) {// Adress
					z304address3Reached = true;
				} else if (qName.equalsIgnoreCase(KohaConstants.Z304_ADDRESS_4_NODE)) {// City
					z304address4Reached = true;
				} else if (qName.equalsIgnoreCase(KohaConstants.Z304_EMAIL_ADDRESS_NODE)) {// E-mails
					z304emailReached = true;
				} else if (qName.equalsIgnoreCase(KohaConstants.Z304_TELEPHONE_1_NODE)) {// Phone No.
					z304telephone1Reached = true;
				}
			}
		} else if (parsingCirculations) {
			if (qName.equalsIgnoreCase(KohaConstants.NOTE_NODE)) {
				String type = attributes.getValue(KohaConstants.TYPE_NODE_ATTRIBUTE);

				if (type != null && type.equalsIgnoreCase(KohaConstants.PARAM_CASH))
					noteWithCashParamReached = true;
			}
		} else if (parsingCash) {
			if (qName.equalsIgnoreCase(KohaConstants.OPEN_SUM_NODE)) {
				openSumReached = true;
			} else if (qName.equalsIgnoreCase(KohaConstants.Z31_NET_SUM_NODE)) {
				z31netSumReached = true;
			} else if (qName.equalsIgnoreCase(KohaConstants.Z31_DATE_NODE)) {
				z31DateReached = true;
			} else if (qName.equalsIgnoreCase(KohaConstants.Z31_DESCRIPTION_NODE)) {
				z31descriptionReached = true;
			}
		} else if (parsingBlockOrTraps) {
			if (qName.matches(KohaConstants.PATRON_BLOCK_NODE + "|" + KohaConstants.CONSORTIAL_BLOCK_NODE)) {
				patronOrConsortialblockReached = true;
			} else if (qName.equalsIgnoreCase(KohaConstants.TRANSLATE_CHANGE_ACTIVE_LIBRARY_NODE)) {
				translateChangeActiveLibraryReached = true;
			}
		} else if (parsingRegistration) {
			if (qName.equalsIgnoreCase(KohaConstants.Z305_BOR_STATUS_NODE)) {
				z305borStatusReached = true;
			} else if (qName.equalsIgnoreCase(KohaConstants.Z305_EXPIRY_DATE_NODE)) {
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		if (parsingAddress) {
			...
			if (qName.equalsIgnoreCase(KohaConstants.GET_PAT_ADRS_NODE))
			...
			if (qName.equalsIgnoreCase(KohaConstants.GET_CIRC_ACTS_NODE))
			...
			} else if (qName.equalsIgnoreCase(KohaConstants.Z31_NODE)) {

				if (fineDate != null && fineSum != null)
					user.addAccountDetails(fineDate, fineSum, fineDescription);

			} else if (qName.equalsIgnoreCase(KohaConstants.PAT_CASH_LIST_NODE))
				parsingCash = false;
			...
			if (qName.equalsIgnoreCase(KohaConstants.PAT_BLOCKS_NODE))
			...
			if (qName.equalsIgnoreCase(KohaConstants.PAT_REGISTER_NODE))
	...
	public void characters(char ch[], int start, int length) throws SAXException {
		...
		if (z304address1Reached) {
			...
			BlockOrTrap bot = KohaUtil.parseBlockOrTrap(new String(ch, start, length));
			blockOrTraps.add(bot);
			patronOrConsortialblockReached = false;
		} else if (translateChangeActiveLibraryReached) {
			agencyId = KohaUtil.createAgencyId(new String(ch, start, length));
			translateChangeActiveLibraryReached = false;
	...
	public KohaLookupUserHandler parseAddress() {
		parsingAddress = true;
		return this;
	}

	public KohaLookupUserHandler parseCirculations() {
		parsingCirculations = true;
		return this;
	}

	public KohaLookupUserHandler parseCash() {
		parsingCash = true;
		return this;
	}

	public KohaLookupUserHandler parseBlockOrTraps() {
		parsingBlockOrTraps = true;
		return this;
	}

	public KohaLookupUserHandler parseRegistration() {
