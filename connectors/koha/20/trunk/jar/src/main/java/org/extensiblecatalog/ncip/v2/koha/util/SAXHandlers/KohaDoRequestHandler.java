...
public class KohaDoRequestHandler extends DefaultHandler {
	...
	public KohaDoRequestHandler(String itemIdVal) {
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		if (qName.equalsIgnoreCase(KohaConstants.HOLD_REQUEST_NODE)) {
			...
			if (link != null && link.contains(itemIdToLookFor)) {
				...
				String deleteAttr = attributes.getValue(KohaConstants.DELETE_NODE_ATTRIBUTE);
				if (deleteAttr.equalsIgnoreCase(KohaConstants.YES)) {
					deletable = true;
				} else
					deletable = false;
			}
		...
		} else if (qName.equalsIgnoreCase(KohaConstants.NOTE_NODE)) {
			noteReached = true;
