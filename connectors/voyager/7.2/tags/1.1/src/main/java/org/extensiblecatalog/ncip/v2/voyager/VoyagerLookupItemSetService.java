...
public class VoyagerLookupItemSetService implements LookupItemSetService {
    ...
    private final int MAX_ITEMS_TO_RETURN =
            Integer.parseInt((String) voyagerConfig.getProperty(VoyagerConstants.CONFIG_VOYAGER_MAX_LUIS_ITEMS));
        ...
        for (BibliographicId bibId : bibIds) {
            ...
            try {
                ...
                if (!checkValidAgencyId(itemAgencyId)) {
                    log.error("Unrecognized Bibliographic Record Agency Id");
                    problems.addAll(ServiceHelper.generateProblems(Version1GeneralProcessingError.NEEDED_DATA_MISSING,
                            null, itemAgencyId, "Unrecognized Bibliographic Record Agency Id"));
...
//    }
    ...
    private List<String> getItemIdsFromHoldingDoc(String holdingId, Document doc) throws ILSException {
        ...
        try {
            ...
            List<Element> itemElements = xpath.selectNodes(doc);
            for (Element itemElement : itemElements) {
                href = itemElement.getAttributeValue("href");
                log.debug("Found href " + itemElement.getAttributeValue("href"));
                index = href.lastIndexOf("/");
                itemIds.add(href.substring(index).substring(1));
                log.debug("Found item id from href: " + href.substring(index).substring(1));
            }
        } catch (JDOMException e) {
            throw new ILSException(e);
        }
    ...
    private Document getHoldingRecordsFromXml(BibliographicId bibliographicId) {
        ...
        // The URL of the web services server + service name
        String ubPrefix;
        //boolean multipleTomcats = new Boolean((Boolean) voyagerConfig.getProperty(VoyagerConstants.CONFIG_VOYAGER_MULTIPLE_TOMCATS)).booleanValue();
        boolean multipleTomcats = (Boolean.parseBoolean((String)voyagerConfig.getProperty(VoyagerConstants.CONFIG_VOYAGER_MULTIPLE_TOMCATS)));
        if (multipleTomcats)
            ubPrefix = "/" + itemAgencyId.toLowerCase() + "/vxws";
        else
