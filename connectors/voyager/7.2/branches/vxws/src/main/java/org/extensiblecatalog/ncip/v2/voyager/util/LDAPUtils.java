...
public class LDAPUtils 
{
	 ...
	 */
	public DirContext getLDAPConnection(String username, String password) throws ILSException
	{
		...
		{						
			...
			String startLocation = NCIPConfiguration.getProperty(VoyagerConstants.CONFIG_EXTERNAL_LDAP_START);
		...
		catch(NamingException e)
		{
			...
			if(e.getMessage().contains("error code 49"))
			{
				...
				throw new ILSException("The username or password was incorrect.");
			}
			else if(e.getMessage().contains("error code 32"))
			{
				...
				else
				{
					...
					throw new ILSException("An error occurred while connecting to the LDAP server defined in the configuration file.");					
				}
			}
	...
	public Properties getGenericLDAPProperties()
	{
		...
		String url = NCIPConfiguration.getProperty(VoyagerConstants.CONFIG_EXTERNAL_LDAP_LOCATION);
		String  port = NCIPConfiguration.getProperty(VoyagerConstants.CONFIG_EXTERNAL_LDAP_PORT);
